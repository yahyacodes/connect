...
import jason from "../Images/adobe-stock.jpg";
import lady from "../Images/lady.jpg";
import jonathan from "../Images/Jonathan.jpg";

const Testimonials = () => {
  const [jasonText, setJasonText] = useState(false);

  const showJasonText = () => {
    setJasonText(!jasonText);
  };

  const [startShow, setShow] = useState(false);

  const showText = () => {
    setShow(!startShow);
  };

  const [jonathanText, setJonathanText] = useState(false);

  const showJonathanText = () => {
    setJonathanText(!jonathanText);
  };
          ...
          {showJasonText && (
            <>
              ...
              <p className="text-md text-gray-500">
                When I first heard of Connect I was not sure if I can try it
                out, but later on I had the courage to sit down and open an
                account where it was cheaper than I expected. Eventually after I
                opened my account and started to engage with others.
              </p>
              <p className="text-md text-gray-500">
                I learned a lot from the sellers I engaged with, which I started
                to try it in business it worked and I gained a lot of customers
                in that period I have been using Connect it really helped me so
                much, because I started by online business few months before I
                discovered Connect and my business was struggling but now my
                business is running well thanks to Connect.
              </p>
            </>
          )}
